...
y0_hypoxia <- apply(exprs(celExpst_ratio_smooth_hypoxia), 2, upperBoundNull)
...
##calculate ratios between ip, controls on LOESS normalised data
celExpst_loess_ratio_normoxia <- getRatio(celExpst_loess, ipNormoxia, controlHypoxia, description="Normoxia-vs-Hypoxia", fkt=median, featureData=F)
...
##remove all unknowns in smoothed ratios (e.g. where values are not available for one condition)
y0_normoxia <- apply(exprs(celExpst_ratio_smooth_normoxia), 2, upperBoundNull)
